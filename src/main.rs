...
#[cfg(target_os = "macos")]
use objc2::{msg_send, class, define_class, ClassType};
#[cfg(target_os = "macos")]
...
struct TrayMenu {
    tray: TrayIcon,
    #[cfg(target_os = "macos")] _delegate: Retained<MenuDelegate>,
...
impl TrayMenu {
    fn new(paths: &AppPaths, auto_connect: bool) -> Result<Self> {
        ...
        let tray = TrayIconBuilder::new()
            ...
            .with_icon_as_template(true).with_tooltip("ProxyBear")
        ...
        if let Some(si) = tray.ns_status_item() {
            unsafe { let m: Retained<AnyObject> = msg_send![&si, menu]; let _: () = msg_send![&m, setDelegate: &*delegate]; }
        }
    ...
    fn set_icon_state(&self, state: TrayIconState) -> Result<()> {
        if self.icon_state.get() == state { return Ok(()); }
        self.tray.set_icon_with_as_template(Some(icons::tray_icon(state)?), true)?;
        self.icon_state.set(state); Ok(())
    }
}
...
impl ProxyBear {
    fn new() -> (Self, iced::Task<Message>) {
        #[cfg(target_os = "macos")]
        unsafe {
            let ns_app: *mut objc2::runtime::AnyObject = msg_send![class!(NSApplication), sharedApplication];
        ...
        (Self {
            ...
            proxy: None, tray, form, stats_text: String::new(), config_path,
    ...
    fn update(&mut self, msg: Message) -> iced::Task<Message> {
        match msg {
            ...
            Message::ProxyDone(err) => {
                ...
                if let Some(e) = err { self.stats.set_error(e); }
                self.update_icon();
                iced::Task::none()
            }
            Message::Tick => {
            ...
            Message::Window(id, ev) => {
                if matches!(ev, iced::window::Event::Closed) && self.settings_window == Some(id) {
                    self.settings_open = false; self.settings_window = None;
                }
                iced::Task::none()
            }
        }
    ...
    fn view(&self, window: iced::window::Id) -> iced::Element<'_, Message> {
        if Some(window) == self.settings_window && self.settings_open {
            return settings::view(&self.form, &self.stats_text, &self.config_path).map(Message::Field);
        }
...
#[derive(Hash)] struct MenuSubId;
#[derive(Hash)] struct ProxySubId;
...
impl ProxyBear {
    fn handle_menu(&mut self, action: MenuAction) -> iced::Task<Message> {
        match action {
            MenuAction::MenuOpened => { self.refresh_stats(); iced::Task::none() }
            MenuAction::StartStop => {
                if self.proxy.is_some() { self.stop_proxy(); iced::Task::none() }
    ...
    fn handle_field(&mut self, field: SettingsField) -> iced::Task<Message> {
        match field {
            ...
            SettingsField::ChooseKey => { self.save_settings(); self.choose_key(); }
        }
        iced::Task::none()
    }
...
impl ProxyBear {
    fn refresh_stats(&mut self) {
        ...
        self.stats_text = format!("{} | {} active | up {} | down {}",
            stats.status, stats.active_connections,
            format_bytes(stats.bytes_up), format_bytes(stats.bytes_down));
        ...
        if s != self.last_status { self.tray.status.set_text(&s); self.last_status = s; }
        let l = format!("{} total, {} active, up {}, down {}",
            stats.total_connections, stats.active_connections,
            format_bytes(stats.bytes_up), format_bytes(stats.bytes_down));
        if l != self.last_stats { self.tray.stats.set_text(&l); self.last_stats = l; }
        let c = config_summary(&config);
        if c != self.last_config { self.tray.config.set_text(&c); self.last_config = c; }
...
impl ProxyBear {
    fn start_proxy(&mut self) -> iced::Task<Message> {
        if self.proxy.is_some() { return iced::Task::none(); }
        if let Err(e) = self.config.lock().unwrap().validate_ready() {
            self.stats.set_error(e.to_string()); return iced::Task::none();
        }
        let (shutdown_tx, shutdown_rx) = oneshot::channel();
        let config = Arc::clone(&self.config); let paths = self.paths.clone();
        let stats = Arc::clone(&self.stats);
        stats.set_status("Starting"); stats.clear_error();
        ...
        self.proxy = Some(ProxyHandle { shutdown: Some(shutdown_tx), task });
        self.update_icon();
        iced::Task::none()
    }
    ...
    fn stop_proxy(&mut self) {
        if let Some(mut p) = self.proxy.take() {
            if let Some(s) = p.shutdown.take() { let _ = s.send(()); }
            p.task.abort();
        }
        self.stats.set_status("Stopped"); self.update_icon();
    }

    fn save_settings(&self) {
        let mut config = self.config.lock().unwrap().clone(); let f = &self.form;
...
fn config_summary(config: &AppConfig) -> String {
    if config.server.is_empty() { "No server configured".into() }
    else { format!("{}@{}:{} -> {}", config.username, config.server, config.port, config.local_addr) }
}

fn format_bytes(bytes: u64) -> String {
    const U: [&str; 4] = ["B", "KiB", "MiB", "GiB"];
    let mut size = bytes as f64; let mut unit = 0;
    while size >= 1024.0 && unit < U.len() - 1 { size /= 1024.0; unit += 1; }
    if unit == 0 { format!("{bytes} B") } else { format!("{size:.1} {}", U[unit]) }
}

fn main() -> iced::Result {
    iced::daemon(ProxyBear::new, ProxyBear::update, ProxyBear::view)
        .title("ProxyBear").subscription(ProxyBear::subscription).run()
}
